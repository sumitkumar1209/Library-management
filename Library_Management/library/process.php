...
$conn = new mysqli($servername, $username, $password, $dbname)or die("cannot connect");
	...
	//	echo 'sdf';
	    ...
	    if($result->num_rows > 0){
          $qry = "update book_copy set copies = copies-1 where title = '$title' and copies > 0";		  
		  $result = $conn->query($qry);
		        $qry1 = "update book_copy set count = count-1 where title = '$title' and count > 0";		  
                $result = $conn->query($qry1);
		   $sql = "INSERT INTO loan_stud (book_id, roll_no, date_expected) VALUES ('$book_id', '$rn', '$date1')";
